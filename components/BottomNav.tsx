...
import { useAppStore } from "@/lib/store"; // 👉 引入 store
...
export function BottomNav() {
  ...
  const { hideBottomNav } = useAppStore(); // 👉 获取开关状态

  // 在某些页面隐藏导航栏; 增加判断：如果路由匹配，或者全局开关打开了，就隐藏
  const hiddenPaths = ["/quiz"];
  if (hiddenPaths.some((path) => pathname.startsWith(path)) || hideBottomNav) {
    return null;
  }
        ...
        return (
          ...
          >
            <Icon
              ...
              style={{
                filter: isActive
                  ? "drop-shadow(0 0 8px rgba(229, 9, 20, 0.5))"
                  : "none",
              }}
            />
            {isActive && (
