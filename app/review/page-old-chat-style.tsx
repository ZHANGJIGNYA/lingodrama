...
import {
  romanceReviewScript,
  workplaceReviewScript,
  sliceOfLifeReviewScript,
} from "@/lib/mockStories";
...
const getRandomGenre = (): "romance" | "workplace" | "slice_of_life" => {
  const genres: Array<"romance" | "workplace" | "slice_of_life"> = [
    "romance",
    "workplace",
    "slice_of_life",
  ];
...
function HighlightedText({
  ...
  const validHighlightedWords = highlightedWords.filter(
    (hw) => !commonWords.includes(hw.word.toLowerCase()),
  );

  validHighlightedWords.forEach((hw, index) => {
    const wordIndex = result
      .toLowerCase()
      .indexOf(hw.word.toLowerCase(), lastIndex);
    ...
    if (wordIndex !== -1) {
      ...
      // 根据用户设置选择释义
      const definition =
        userSettings?.definition_preference === "simple_english"
...
export default function ReviewPage() {
  ...
  const [currentGenre, setCurrentGenre] = useState<
    "romance" | "workplace" | "slice_of_life"
  >(getInitialGenre());
  ...
  const generateScript = async (
    ...
    try {
      ...
      console.log(
        ...
        vocabularyList.map(
          (v) =>
            `${v.word} (mastery: ${v.mastery_level}%, next: ${v.next_review_date})`,
        ),
      );

      // 使用艾宾浩斯算法获取今日待复习单词
      const wordsPerReview = userSettings?.words_per_review || 3;
      const wordsToReview = getTodayReviewWords(vocabularyList).slice(
        0,
        wordsPerReview,
      );
      ...
      if (wordsToReview.length === 0) {
        ...
        setGenerationError(
          `⚠️ No words to review today. ${vocabularyList.length > 0 ? "All words mastered! 🎉" : "Add words in Vault first."}`,
        );
      ...
      console.log(
        `🎬 Generating script for ${wordsToReview.length} words:`,
      ...
      // 清理并验证消息
      const cleanedMessages = data.script.messages.map(
        (msg: ChatMessage, index: number) => {
          ...
          if (originalContent !== cleanedContent) {
            console.warn(
              `⚠️ Message ${index + 1} contained HTML tags (cleaned):`,
            );
      ...
      console.log(
        `✅ Script generated successfully with ${data.provider || "AI"}!`,
      );
      ...
      if (error.message?.includes("API key")) {
        ...
        errorMsg =
          "⏳ Claude API is busy. Trying backup model or using demo story...";
      } else if (
        ...
        errorMsg =
          "😔 All AI models are busy. Using demo story. Try again in a few minutes.";
      }
  ...
  const isComplete = script
    ? currentMessageIndex === script.messages.length - 1
    : false;
        ...
        >
          ...
          <div className="mb-6 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(genreTheme) as Array<keyof typeof genreTheme>).map(
                (genre) => (
                  <button
                    key={genre}
          ...
          {!script && !isGenerating && (
            <div className="bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-3 mb-6">
              <p className="text-xs text-gray-600 dark:text-gray-400">
                ⚙️ Words per session: {userSettings?.words_per_review || 3}{" "}
                (Change in Settings)
              </p>
            </div>
          )}
          ...
          <motion.button
            ...
            className={`bg-blue-500 text-white px-8 py-4 rounded-xl font-semibold text-lg flex items-center gap-2 mx-auto transition-all shadow-lg ${
              isGenerating
                ...
                : "hover:bg-blue-600 shadow-blue-500/30"
            }`}
          >
            {isGenerating ? (
              <>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      {/* 消息区域 */}
      <div className="flex-1 overflow-y-auto px-4 py-6 pb-32 bg-gray-50 dark:bg-gray-900">
